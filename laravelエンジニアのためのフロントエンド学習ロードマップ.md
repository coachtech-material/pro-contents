...
1.  **🎯 このセクションで学ぶこと**: 学習目標を箇条書きで2〜3項目明示する。
2.  **導入**: なぜこの技術が必要か、実務での具体例を挙げて説明する。
3.  **概念の説明**: 比喩を交えつつ、技術の概要を分かりやすく解説する。
4.  **詳細解説**: コマンドやコードの使い方、オプション、注意点を詳細に説明する。
5.  **💡 TIP**: 補足情報、よくある間違い、ベストプラクティスを提示する。
6.  **🏃 実践**: ステップバイステップのハンズオン形式で、学習者が実際に手を動かせるようにする。
7.  **コードリーディング**: 実践パートで使ったコードや実行結果を1行ずつ丁寧に解説する。
8.  **✨ まとめ**: セクションの要点を3〜5項目で箇条書きで整理する。
9.  **📝 学習のポイント**: 学習者がセクションの理解度を自己評価できるチェックリストを3〜5項目で提供する。
