...
このセクションで紹介した `repeat(auto-fit, minmax(250px, 1fr))` のパターンを使って、シンプルなカードギャラリーを作ってみましょう。3〜6枚程度のカード（適当な `<div>` 要素）を並べたコンテナを用意し、ブラウザの幅を変えるとカードの列数が自動的に増減することを確認してください。
